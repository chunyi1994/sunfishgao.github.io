...
科学上网（翻墙）其实就是一个国外服务器帮你访问一些无法访问的网站，再把网站内容发给你的一个过程。本身实现其实并不复杂，成本也比较低廉。编写这类程序，需要掌握TCP知识，掌握一款网络库使用，了解加密解密等等。虽然科学上网用得最多的是看web，但这里确实不涉及HTTP知识。
...
### 3.1 多线程同步阻塞式
这里讨论的多线程同步阻塞式，并不是真的用pthread里的线程去写并发。因为一个线程栈可能8m，线程一多内存就撑不住了。再加上线程切换的开销，使得这种传统的并发方式连10k都够呛。</br>
...
#### “异步”代码的“保存上下文”
无论是Proactor还是Reactor，凡是接近nodejs那种异步的写法，都需要主动保存“上下文”在自己的内存中。read函数一次性读的字节数也是不确定的，有时需要多次调用read才能接受完完整的数据。所以没接收完整之前需要把他们保存起来。在下次接收的时候继续append数据，这就是所谓“保存上下文”。必要时还需要使用状态机保存状态。
